...
# After subtracting known OS chrome (below), a further 5 % margin is kept as
# breathing room so the window never sits flush against a screen edge.
_SCREEN_MARGIN: float = 0.95


def _system_chrome() -> tuple[int, int]:
    """Return (top_px, bottom_px) pixel estimates for OS UI chrome.

    These are subtracted from the raw display height before computing the
    available area for maze windows, so max-size clamping and window
    centering both stay within the truly usable portion of the screen.
    """
    if sys.platform == "darwin":
        return 30, 80   # menu bar (~25 px) + Dock (variable; 80 is conservative)
    if sys.platform.startswith("linux"):
        return 50, 0    # typical top panel / taskbar
    return 0, 0


def _usable_area(sw: int, sh: int) -> tuple[int, int]:
    """Return (avail_w, avail_h) after removing OS chrome and the safety margin."""
    top, bottom = _system_chrome()
    return int(sw * _SCREEN_MARGIN), int((sh - top - bottom) * _SCREEN_MARGIN)


def _center_window(sw: int, sh: int) -> None:
    """Reposition the pygame window so it is centered in the usable screen area.

    Keeps the window below the macOS menu bar / Linux panel (top chrome) and
    above the macOS Dock (bottom chrome).  Silent no-op in headless environments.
    """
    top, bottom = _system_chrome()
    ...
    if surf is None:
        return
    ww, wh = surf.get_size()
    usable_h = sh - top - bottom
    x = max(0, (sw - ww) // 2)
    y = max(top, top + (usable_h - wh) // 2)
    try:
        pygame.Window.from_display_module().position = (x, y)
    except Exception:
        pass  # headless / SDL_VIDEODRIVER=dummy / older pygame builds
...
def main(argv: list[str] | None = None) -> None:
    ...
    _avail_w, _avail_h = _usable_area(_sw, _sh)
            ...
            if new_request is not None:
                try:
                    maze, renderer, cell_size, screen = _apply_new_request(
                        new_request, maze, screen, _sw, _sh
                    )
                    request = new_request
